const API_URL = "http://localhost:4000";//https://tachmanagment-back.onrender.com //http://localhost:4000
...
export async function createProject(projectData, session) {
  ...
  const timeout = setTimeout(() => {
    ...
    controller.abort();
  }, 5000); // 5s pour tester plus rapidement
...
export async function getUserGroupsWithDetails(userId, session) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data.message || 'Failed to fetch user groups with details');
  return data;
}
